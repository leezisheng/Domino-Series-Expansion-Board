...
class BusStepMotor:
    ...
    def _next_step(self, motor_id: int) -> None:
        ...
        if abs(self.step_counters[motor_id]) >= self.steps[motor_id]:
            self.stop_step_motion(motor_id)
    ...
    def _start_timer(self, motor_id: int, speed: int) -> None:
        ...
        # 使用 lambda 函数来包裹 _next_step，并在调用时传递 motor_id
    ...
    def start_continuous_motion(self, motor_id: int, direction: int, driver_mode: int, speed: int) -> None:
        ...
        Raises:
            ValueError: 如果速度不是0到10000之间的整数，或者电机编号无效，或者电机方向无效，或者驱动模式无效。
        ...
        Raises:
            ValueError: If speed is not an integer between 0 and 10000, or motor ID is invalid, or motor direction is invalid, or driver mode is invalid.
        """
        # 判断速度是不是在0到10000之间的整数
        if not isinstance(speed, int) or speed < 0 or speed > 10000:
            raise ValueError("Invalid speed: %d. Speed must be an integer between 0 and 10000." % speed)

        # 判断电机ID是否有效
        if motor_id < 0 or motor_id >= self.motor_count:
            raise ValueError("Invalid motor_id: %d. Motor ID must be between 0 and %d." % (motor_id, self.motor_count - 1))
    ...
    def stop_continuous_motion(self, motor_id: int) -> None:
        ...
        if motor_id < 0 or motor_id >= self.motor_count:
            raise ValueError(
                "Invalid motor_id: %d. Motor ID must be between 0 and %d." % (motor_id, self.motor_count - 1))
    ...
    def start_step_motion(self, motor_id: int, direction: int, driver_mode: int, speed: int, steps: int) -> None:
        ...
        Raises:
            ValueError: 如果目标步数不是正整数，或者速度不是0到10000之间的整数，或者电机编号无效，或者电机方向无效，或者驱动模式无效。
        ...
        Raises:
            ValueError: If the target steps is not a positive integer, or the speed is not an integer between 0 and 10000,
    ...
    def stop_step_motion(self, motor_id: int) -> None:
        ...
        base_channel = motor_id * 4
        for i in range(4):
            self.pca9685.duty(base_channel + i, 0)
